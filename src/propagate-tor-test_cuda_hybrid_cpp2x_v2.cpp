...
void dispatch_p2r_kernels(auto&& p2r_kernel, auto&& pref_to_device, auto&& pref_to_host, stream_tp compute_stream, const int nb_, const int nevnts_){
  ...
  // First describe main compute task:
  auto compute_p2r_task = ex::just() | exec::on(compute_stream, ex::then([&, len = nb_*nevnts_] {	  
    auto policy = std::execution::par_unseq;
    //
    auto outer_loop_range = std::ranges::views::iota(0, len*(is_cuda_target ? bSize : 1));
    //
    std::for_each(policy,
  ...
  // Now, describe prefetch tasks:
  auto pref_to_dev_task = ex::just() | ex::then(pref_to_device);

  auto pref_to_hst_task = ex::just() | ex::then(pref_to_host);

  // Finally, describe a joined task:
  ex::sender auto joined_task = ex::when_all(pref_to_dev_task, compute_p2r_task, pref_to_hst_task);

  // Ready to launch the joined task:
  stdexec::this_thread::sync_wait(std::move(joined_task));
}
...
int main (int argc, char* argv[]) {
   ...
   if constexpr  (enable_cuda_launcher) {
     ...
     std::cout << "Running with stdexec launcher.." << std::endl;
   }
#endif
...
#ifndef cuda_launcher 
   // Initialize scheduler
   exec::static_thread_pool ctx{3};
   
   stdexec::scheduler auto sch = ctx.get_scheduler();//!
   const auto& compute_queue = sch;
#else
   const auto& compute_queue = stream;
#endif   
   ...
   for(int itr=0; itr<NITER; itr++) {
     ...
     //
     dispatch_p2r_kernels<bsize, decltype(compute_queue), enable_cuda>(p2r_kernels, prefetch_to_device, prefetch_to_host, compute_queue, nb, nevts);
     //
